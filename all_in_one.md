...
## 📌 Assignment 1: Information Retrieval Basics
...
*(Note: Diagram should show the flow from query input → processing → matching → ranking → output)*
...
**Answer:**  
Conflation Algorithm is used to reduce words to their root or base form by removing suffixes and prefixes. It groups related words together (e.g., "running," "runs," "ran" → "run"). This process is also known as **stemming** and helps in improving search accuracy by matching different forms of the same word.
...
- "some" → "some" (no change)
- "text" → "text" (no change)
- "is" → "is" (stop word, may be removed)
- "here" → "here" (no change)
...
## 📌 Assignment 2: Clustering in Information Retrieval
...
**Answer:**  
- **Time Complexity:** O(n×k), where n = number of documents, k = number of clusters
- **Space Complexity:** O(n×m), where n = number of documents, m = number of features/terms
...
- **Handling variants** (e.g., through stemming: "run," "running" → "run")
...
**Answer:**  
**Time Complexity:**
- **Building index:** O(n×m), where n = number of documents, m = average document length
...
## 📌 Assignment 4: Precision and Recall
...
## 📌 Assignment 5: F-Measure and E-Measure
...
F-Measure = 2 × (0.8 × 0.6) / (0.8 + 0.6) = 0.686 or 68.6%
...
**Answer:**  
**E-Measure** (also called **Fβ-Measure**) is a **weighted harmonic mean** of Precision and Recall that allows adjusting the relative importance of precision versus recall using a parameter β.

**Formula:**  
```
E-Measure = (1 + β²) × (Precision × Recall) / (β² × Precision + Recall)
```

**Where:**
- **β < 1:** Emphasizes Precision (precision is more important)
- **β = 1:** Equal weight (becomes standard F-Measure)
- **β > 1:** Emphasizes Recall (recall is more important)

**Common variants:**
- **F₀.₅:** Weights precision twice as much as recall
- **F₁:** Standard F-measure (equal weights)
- **F₂:** Weights recall twice as much as precision
...
4. **Application-Specific Tuning:**
   - **E-commerce:** Higher precision (β < 1) to avoid showing irrelevant products
   - **Medical research:** Higher recall (β > 1) to not miss important studies
   - **General search:** Balanced approach (β = 1)
...
## 📌 Assignment 6: Image Processing & Histograms
...
2. **Access pixel array** (typically 3D array: height × width × 3)
3. **Separate channels:**
...
- Saves time (one query → multiple engines)
...
## 📌 Assignment 8: Weather API Integration
...
## 📌 Assignment 9: Recommender Systems
...
```
Million Items → Relevance Estimation → Top 100 Candidates → 
Personalized Ranking → Top 10 Recommendations
```
...
3. **Scalability:**
   ...
   - Millions of users × millions of items = billions of comparisons
...
```
Final_Score = w1 × CF_Score + w2 × CBF_Score + w3 × Popularity_Score
Example: Score = 0.6 × CF + 0.3 × CBF + 0.1 × Popularity
```
...
Step 2: CBF ranks these 100 → Top 10
```
...
- Example: Buying phone → suggests phone case, screen protector
- Increases cart value through relevant suggestions

**"Customers Who Bought This Also Bought":**
- Item-based collaborative filtering
- Discovers related products you might need
- Example: Buying camera → suggests memory card, camera bag
...
- Example: Viewed 5 different shoes → shows more shoes in similar style
...
- Example: Buying laptop → suggests mouse, laptop sleeve
...
- Example: Bought DSLR camera → suggests lenses, tripod
...
→ Recommend Item3 to User B
```

**Item-to-Item CF:**
```
50% of buyers who purchased "iPhone 15" also bought "iPhone case"
→ When you add iPhone to cart, suggest case
```

**Content-Based:**
```
You bought: "Nike Running Shoes, Size 10, Blue"
→ Recommend: Other Nike shoes, Size 10, Sports shoes, Blue products
```

**Hybrid Approach:**
```
Weight = 0.4 × Collaborative + 0.3 × Content + 0.2 × Popularity + 0.1 × Trending
```
...
3. **Discovery of New Products:**
   ...
   - **Example:** Buying coffee maker → discover specialty coffee beans you'd love
...
7. **Complete Purchase:**
   ...
   - **Example:** Buying printer → reminded to buy ink cartridges
...
**Step 2 - Search:**
- Type "laptop" → autocomplete suggests "laptop for programming under 60000"
- Search results ranked by your preferences (RAM, processor priority)

**Step 3 - Product Page:**
- View Dell XPS 15
- See "Frequently bought together" → Mouse, laptop bag, screen protector (Bundle discount!)
- "Compare with similar items" → 3 similar laptops side-by-side
- "Customers also viewed" → HP Pavilion, Lenovo ThinkPad
...
- "Don't forget" → Laptop cooling pad, external hard drive
- "Complete your purchase" → Laptop sleeve, HDMI cable

**Step 6 - Checkout:**
- "Customers who bought this also bought" → Wireless mouse at special price
...
- **Overwhelming choices** → **Curated selections**
- **Generic store** → **Personal shopping assistant**
- **Random browsing** → **Targeted suggestions**
...
**Best of luck with your viva examination! 🎓**
